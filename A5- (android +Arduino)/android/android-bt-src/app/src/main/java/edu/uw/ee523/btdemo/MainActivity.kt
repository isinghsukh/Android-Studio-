...
//        binding.recyclerViewGatt.adapter = myBtGattListAdapter
    ...
    private val bluetoothGattCallback = object : BluetoothGattCallback() {
        ...
        override fun onCharacteristicChanged(
            ...
            var value: Int? = characteristic?.value?.get(1)?.toString()?.toInt()
//            var value: Double? = characteristic?.value?.get(1)?.toDouble()
            Log.i(TAG, "Value: $value")
//            characteristic?.value?.get(1)?.toString().let {
//                if (it != null) {
//                    Log.i(TAG, it)
//                }
//            }
                ...
                if(value in 0..50) {
                    handler.post {
                        binding.textViewStatus.text =
                            "Temp from Circuit Playground: $value"
                    }
                } else {
                    handler.post {
                        binding.textViewStatus.text =
                            "CIrcuit Playground button pressed: " + characteristic?.value?.get(1)?.toString()?.toInt()?.toChar()
                    }
                }
            }
//            handler.post {
//                binding.textView2Status.text =
//                    "New val read: " + characteristic2?.value?.get(1)?.toUByte().toString()
//            }
        }
    ...
    private fun checkAndConnectToHRM(services: List<BluetoothGattService>?) {
        ...
        services?.forEach { service ->
            if (service.uuid == SampleGattAttributes.TEMPERATURE_SERVICE_UUID){
                Log.i(TAG, "Found HRM Service")
                val characteristic = service.getCharacteristic(SampleGattAttributes.TEMPERATURE_MEASUREMENT_UUID)
                bluetoothGatt?.readCharacteristic(characteristic)

                characteristic2 = service.getCharacteristic(SampleGattAttributes.BUTTON_MEASUREMENT_UUID)
...
////                    BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE
////                bluetoothGatt!!.writeDescriptor(descriptor)
//                bluetoothGattCallback.onDescriptorWrite(bluetoothGatt, descriptor2, 1)
//                // When the characteristic value changes, the Gatt callback will be notified
